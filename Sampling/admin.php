<?php
session_start();//continues the session containing prevouse information

if (!isset($_SESSION['username'])) { //if not set, it means that admin hasn't logged in
    header("Location: logIn.php"); //redirects users to login page
    exit;
}
// echo $_SESSION['userId'];
?>
<!DOCTYPE html>
<html>
    <head>
        <title>Admin</title>
        <script src="https://code.jquery.com/jquery-3.1.1.min.js" integrity="sha256-hVVnYaiADRTO2PzUGmuLJr8BLUSjGIZsDYGmIJLv2b8=" crossorigin="anonymous"></script>
        <link rel="stylesheet" href="css/layout.css" type="text/css" />
        <link rel="stylesheet" href="css/inventory.css" type="text/css" />
    </head>
    <style>
        .smlrTitle{
           font-size:16px;
        }
    </style>
    <body>
        <div id="bodyWrapper">
            <div class="mainDivs" id="nav">
                    <ul id="homeNav">
                        <li><a id="homeBtn" class="navBtn" href="home.php">Home</a></li>
                    </ul>
                    <ul id="pagesNav">
                      <li><a class="navBtn" href="samplingRequests.php">Sampling Orders</a></li>
                      <li><a class="navBtn" href="collectedSamples.php">Collected Samples</a></li>
                      <li><a class="navBtn" href="sampleReports.php">Sampling Reports</a></li>
                      <li><a class="navBtn active" href="admin.php">Admin</a></li>
                    </ul>
                    <ul id="logOutNav">
                        <li id="logOutBtn"><a class="navBtn" href="logout.php">Log Out</a></li>
                    </ul>
            </div>
            
            <div class="mainDivs" id="collectiveInfo">
                <div id="search">
                    <div id="searchInputDiv" class="searchBox1">
                        <input id="searchInput" type="text" name="search"/>
                    </div>
                    <div id="searchBtnDiv" class="searchBox1">
                        <button id="searchBtn">|Search</button>
                    </div>
                </div>
                <div id="records">
                    <div id="inventory">
                        <div class="title">
                            <div>Admin</div>
                        </div>
                        <div class="table">
                            <div id="currentInventory">
                                <div class="title smlrTitle">Users</div>
                                <table id="users"></table><br>
                                <div class="title smlrTitle">Customers</div>
                                <table id="customers"></table>
                            </div>
                        </div>
                    </div>
                </div>
                <div id="footer1" class="footer"><center>PRIORITY SAMPLING</center></div>
            </div>
            
            <div class="mainDivs" id="individualInfo">
                <div id="dateTimeInfo">
                    <div class="dateUserInfo" id="logo"><img id="logoImg" src="images/logo.png"></img></div>
                    <div class="dateUserInfo" id="blankSpace"></div>
                    <div class="dateUserInfo" id="dayUserInfo">
                        <div id="currentDate"></div>
                        <div id="currentTime"></div>
                        <div id="userNameLName"><?= $_SESSION[adminFullName]?></div>
                    </div>
                </div>
                
                <div id="recordInfo">
            <!--UPDATING RECORDS POPUP WINDOWS-->        
                    <div id="updateUser" title="Dialog Title" class="infoSection popUpBox ">
                            <form class="popup-content animate">
                            <div class="infoSection">Updating User</div>
                                
                                <div class="userId" style="display:hidden"></div>
                                
                                <label for="name">Name</label>
                                <input type="text" id="updtUserName" name="name"/><br>
                                <label for="lName">Last Name</label>
                                <input type="text" id="updtUserLName" name="lName"/><br>
                                <label for="username">Username</label>
                                <input type="text" id="updtUsername"  name="username"/><br>
                                <label for="password">Password</label>
                                <input type="password" id="updtUserPass" name="password"/><br>
                                <label for="clearance">Clearance</label>
                                <input type="text" id="updtUserClear" name="clearance"/><br>
                                
                                <input type="submit" onclick='updateUser()' value="Update" name="updateForm">
                                <button id="cancelbtn" type="button"  onclick="document.getElementById('updateUser').style.display='none'" class="cancelbtn">Cancel</button>
                            </form>
                    </div>
                    <div id="updateCustomer" title="Dialog Title" class="infoSection popUpBox ">
                            <form class="popup-content animate">
                            <div class="infoSection">Updating Customer</div>
                            
                                <div class="customerId" style="display:hidden"></div>
                                
                                <label for="customerName">Name: </label>
                                <input id="customerName" type="text" name="customerName"/><br>
                                <label for="custcustomerRepNameRepName">Rep Name: </label>
                                <input id="customerRepName" type="text" name="customerRepName"/><br>
                                <label for="customerLocation">Location: </label>
                                <input id="customerLocation" type="text" name="customerLocation"/><br>
                                <label for="customerPhone">Representative Phone: </label>
                                <input id="customerPhone" type="text" name="customerPhone"/><br>
                                <label for="customerEmail">Representative Email: </label>
                                <input id="customerEmail" type="text" name="customerEmail"/><br>
                                
                                <input type="submit" onclick='updateCustomer()' value="Update" name="updateForm">
                                <button id="cancelbtn2" type="button"  onclick="document.getElementById('updateCustomer').style.display='none'" class="cancelbtn">Cancel</button>
                            </form>
                    </div>
            <!--ADDING RECORDS POPUP WINDOWS-->
                    <div id="addUser" title="Dialog Title" class="infoSection popUpBox ">
                            <form class="popup-content animate">
                            <div class="infoSection">Add User</div>
                            
                                <div class="userId" style="display:hidden"></div>
                                
                                <label for="name">Name</label>
                                <input type="text" id="updtUserName2" name="name2"/><br>
                                <label for="lName">Last Name</label>
                                <input type="text" id="updtUserLName2" name="lName2"/><br>
                                <label for="username">Username</label>
                                <input type="text" id="updtUsername2"  name="username2"/><br>
                                <label for="password">Password</label>
                                <input type="password" id="updtUserPass2" name="password2"/><br>
                                <label for="clearance">Clearance</label>
                                <input type="text" id="updtUserClear2" name="clearance2"/><br>
                                
                                <input type="submit" onclick='insertNewUser()' value="Add" name="updateForm">
                                <button id="cancelbtn3" type="button"  onclick="document.getElementById('addUser').style.display='none'" class="cancelbtn">Cancel</button>
                            </form>
                    </div>
                    <div id="addCust" title="Dialog Title" class="infoSection popUpBox ">
                            <form class="popup-content animate">
                            <div class="infoSection">Add Customer</div>
                            
                                <div class="customerId" style="display:hidden"></div>
                                
                                <label for="custName">Name: </label>
                                <input id="custName2" type="text" name="custName2"/><br>
                                <label for="custRepName">Rep Name: </label>
                                <input id="custRepName2" type="text" name="custRepName2"/><br>
                                <label for="custLocation">Location: </label>
                                <input id="custLocation2" type="text" name="custLocation2"/><br>
                                <label for="custPhone">Representative Phone: </label>
                                <input id="custPhone2" type="text" name="custPhone2"/><br>
                                <label for="custEmail">Representative Email: </label>
                                <input id="custEmail2" type="text" name="custEmail2"/><br>
                                
                                <input type="submit" onclick='insertNewCust()' value="Add" name="updateForm">
                                <button id="cancelbtn4" type="button"  onclick="document.getElementById('addCust').style.display='none'" class="cancelbtn">Cancel</button>
                            </form>
                    </div>
                    
                        
                    <div id="currentRecInfo">
                        <center> <span id="selectRec">&#8678; SELECT A RECORD</span></center>
                        <div id="userRecInfo"></div>
                        <div id="custRecInfo"></div>
                        </div>
                    
                </div>
                
                <div id="footer2" class="footer"><center>POWERD BY HARVEST CUBE</center></div>
            </div>
            
        </div>
    </body>
    
<!--LIBRARY SCRIPTS-->
    <script src="js/moment.js"></script>
    <script src="js/printThis.js"></script>
    <script src="js/jspdf.min.js"></script>

    <script>
    ///////////////ADDING USER DATE AND TIME INFO///////////////   
    
            ////////GETTING THE SIDE C TIME INFO
                $(document).ready(function() {
                    var interval = setInterval(function() {
                        var momentNow = moment();
                        $('#currentDate').html(momentNow.format('dddd') + ': '
                                            + momentNow.format('MMM Do YY'));
                        $('#currentTime').html(momentNow.format('A hh:mm:ss'));
                    }, 100);
                });
        
        
        
    ///////////////GETTING RECORDS///////////////
    
        ////////GETTING Users
                $(document).ready(function() {
                    var users=[];
                    
                    $.ajax({
                        url: "getUsers.php", //the page containing php script
                        type: "get", //request type,
                        dataType: "json",
                        success: function(data) {
                            console.log(data);
                            users=data;
                            $("#users").append("<tr><th>Name</th> <th>LastName</th> <th>Username</th> </tr>");
                            
                            for(var i in users){
                                $("#users").append("<tr class='inventoryRecord' id="+users[i].Id+">  <td>"+users[i].name+"</td> <td>" + users[i].lName +"</td> <td>"+users[i].username+"</td> </tr>");
                            }
                        }
                    });
                    
                 }); 
                 
                
        ////////GETTING Customers
                $(document).ready(function() {
                    var customers=[];
                    
                    $.ajax({
                        url: "getCustomers.php", //the page containing php script
                        type: "get", //request type,
                        dataType: "json",
                        success: function(data) {
                            console.log(data);
                            customers=data;
                            $("#customers").append("<tr><th>Company Name</th> <th>Location</th> <th>Rep Name</th> </tr>");
                            
                            for(var i in customers){
                                $("#customers").append("<tr class='inventoryRecord' id="+customers[i].Id+">  <td>"+customers[i].name+"</td> <td>" + customers[i].location +"</td> <td>"+customers[i].repName+"</td> </tr>");
                            }
                        }
                    });
                    
                 }); 
                
 
    
    ///////////////ADDING MAIN RECORD INFO TO SIDE C//////////////
            
            /////ADDING THE USERS
                $(document).ready(function(){
            $('#users').on('click', function(e){
                console.log("clicked");
                var userId = $(e.target).closest('tr').attr('id');
                console.log(userId);
                
                
                $.ajax({
                    type: "GET",
                    url: "getUserRec.php",
                    data: {
                        "userId":userId,
                    }
                })
                
                .done(function(data) {
                    
                    var users = JSON.parse(data);
                    console.log(users);
                    
            //appending id to div for edit and delete use later
                $('.userId').html("");
                $('.userId').append(users[0].Id);
                $('.userId').css("display", "none");
                    
            //////////Clearing Existing Div INNER Info
                  $('#userRecInfo').html('');  
                  $('#selectRec').html('');
                
            //////////RECORD INFO/////////
            
                //CREATING INVENTORY INFO DIV
                    var userInfo=$("<div></div>");
    				userInfo.attr("id", "userInfo");
    				// collsampInfoSec.attr("class", "userInfo");
    				
    			//CREATING INVENTORY INFO TITLE DIV	
                    var userTitle=$("<div></div>");
    				userTitle.append("User Info");
    				userTitle.attr("class", "secTitle");
    				userTitle.attr("id", "userTitle");
    				userTitle.css("font-size", "30px");
    		      //APPENDING TO INVENTORY DIV	
    				userInfo.append(userTitle);
                    
    			//CREATING INVENTORY INFO AMOUNT DIV	
    				var userName=$("<div></div>");
    				userName.append("Name: "+users[0].name);
    				userName.attr("class", "recInfo");
    				userName.attr("id", "userName");
    			  //APPENDING TO INVENTORY DIV	
    				userInfo.append(userName);
    				
    			//CREATING INVENTORY INFO AMOUNT DIV	
    				var userLName=$("<div></div>");
    				userLName.append("Last Name: "+users[0].lName);
    				userLName.attr("class", "recInfo2");
    				userLName.attr("id", "userLName");
    			  //APPENDING TO INVENTORY DIV	
    				userInfo.append(userLName);
    				
    			//CREATING INVENTORY INFO AMOUNT DIV	
    				var username=$("<div></div>");
    				username.append("Username: "+users[0].username);
    				username.attr("class", "recInfo");
    				username.attr("id", "username");
    			  //APPENDING TO INVENTORY DIV	
    				userInfo.append(username);
    				
    			//CREATING INVENTORY INFO AMOUNT DIV	
    				var clearance=$("<div></div>");
    				clearance.append("Clearance: "+users[0].clearance);
    				clearance.attr("class", "recInfo");
    				clearance.attr("id", "clearance");
    			  //APPENDING TO INVENTORY DIV	
    				userInfo.append(clearance);
    			
    // 			/////APPENDING INVENTORY INFO DIV TO CURRENT RECORD BODY DIV
    				$('#userRecInfo').append(userInfo);
    				
    // 		//////////BUTTONS/////////
    		    
    		    //CREATING THE BUTTON INFO DIV
                 var buttons=$("<div></div>");
    				buttons.attr("id", "buttonSec");
    				buttons.attr("class", "infoSection");
    				
    			//CREATING PRINT/DOWNLOAD BUTTONS	
    				var btnInfo=$("<div></div>");
    				btnInfo.append("<button id='userPrint' onclick='userPrint()'>Print</button> <button id='userPDF' onclick='userPDF()'>Download</button>");
    				btnInfo.attr("class", "recInfo3");
    			  //APPENDING TO INVENTORY DIV	
    				buttons.append(btnInfo);
    				
    			//CREATING EDIT/DELETE BUTTONS	
    				var btnInfo2=$("<div></div>");
    				btnInfo2.append("<button id='userUpdate' onclick ='userUpdate()'>Update </button> <button onclick='userDelete()' id='userDelete'>Delete</button> <button onclick='addUser()' id='addUser'>Add User</button>");
    				btnInfo2.attr("class", "recInfo4");
    			  //APPENDING TO INVENTORY DIV	
    				buttons.append(btnInfo2);
    				
    // 		/////APPENDING INVENTORY INFO DIV TO CURRENT RECORD BODY DIV
    				$('#userRecInfo').append(buttons);
    				
                })
                .fail(function(xhr, status, errorThrown) {
                    console.log("Sorry, there was a problem!");
                })
                .always(function(xhr, status) {
                    console.log("The request is complete!");
                });
                
            })
        });
           
            /////ADDING THE Customers
                $(document).ready(function(){
                    $('#customers').on('click', function(e){
                        var custId = $(e.target).closest('tr').attr('id');
                        console.log(custId);
                        
                        
                        $.ajax({
                            type: "GET",
                            url: "getCustRec.php",
                            data: {
                                "custId":custId,
                            }
                        })
                        
                        .done(function(data) {
                            
                            var customer = JSON.parse(data);
                            console.log(customer);
                            
                    //appending id to div for edit and delete use later
                        $('.customerId').html("");
                        $('.customerId').append(customer[0].Id);
                        $('.customerId').css("display", "none");
                                    
                    //////////Clearing Existing Div INNER Info
                          $('#custRecInfo').html(''); 
                          $('#selectRec').html('');
                        
                    //////////INVENTORY INFO/////////
                    
                        //CREATING INVENTORY INFO DIV
                            var custInfo=$("<div></div>");
            				custInfo.attr("id", "custInfo");
            				// collsampInfoSec.attr("class", "infoSection");
            				
            			//CREATING INVENTORY INFO TITLE DIV	
                            var custTitle=$("<div></div>");
            				custTitle.append("Sample Report Info");
            				custTitle.attr("class", "secTitle");
            				custTitle.attr("id", "custTitle");
            				custTitle.css("font-size", "30px");
            		      //APPENDING TO INVENTORY DIV	
            				custInfo.append(custTitle);
                            
            			//CREATING INVENTORY INFO AMOUNT DIV	
            				var custName=$("<div></div>");
            				custName.append("Customer: "+customer[0].name);
            				custName.attr("class", "recInfo");
            				custName.attr("id", "custName");
            				custInfo.append(custName);
            				
            			//CREATING INVENTORY INFO AMOUNT DIV	
            				var custLoc=$("<div></div>");
            				custLoc.append("Customer: "+customer[0].location);
            				custLoc.attr("class", "recInfo");
            				custLoc.attr("id", "custLoc");
            				custInfo.append(custLoc);
            				
            			//CREATING INVENTORY INFO AMOUNT DIV	
            				var custRepName=$("<div></div>");
            				custRepName.append("Rep Name: "+customer[0].repName);
            				custRepName.attr("class", "recInfo");
            				custRepName.attr("id", "custRepName");
            				custInfo.append(custRepName);
            				
            			//CREATING INVENTORY INFO AMOUNT DIV	
            				var custPhone=$("<div></div>");
            				custPhone.append("Rep Phone#: "+customer[0].phone);
            				custPhone.attr("class", "recInfo");
            				custPhone.attr("id", "custPhone");
            				custInfo.append(custPhone);
            				
            			//CREATING INVENTORY INFO AMOUNT DIV	
            				var custEmail=$("<div></div>");
            				custEmail.append("Rep Email: "+customer[0].email);
            				custEmail.attr("class", "recInfo");
            				custEmail.attr("id", "custEmail");
            				custInfo.append(custEmail);
            			  
            			 // //APPENDING TO INVENTORY DIV	
            				// custInfo.append(actSampAmnt);
            			
            // 			/////APPENDING INVENTORY INFO DIV TO CURRENT RECORD BODY DIV
            				$('#custRecInfo').append(custInfo);
            				
            		//////////BUTTONS/////////
            		    
            		    //CREATING THE BUTTON INFO DIV
                            var buttons=$("<div></div>");
            				buttons.attr("id", "buttonSec");
            				buttons.attr("class", "infoSection");
            				
            			//CREATING PRINT/DOWNLOAD BUTTONS	
            				var btnInfo=$("<div></div>");
            				btnInfo.append("<button id='custPrint' onclick='custPrint()'>Print</button> <button id='custPDF' onclick='custPDF()'>Download</button>");
            				btnInfo.attr("class", "recInfo3");
            			  //APPENDING TO INVENTORY DIV	
            				buttons.append(btnInfo);
            				
            			//CREATING EDIT/DELETE BUTTONS	
            				var btnInfo2=$("<div></div>");
            				btnInfo2.append("<button id='custUpdate' onclick ='custUpdate()'>Update </button> <button onclick='custDelete()' id='custDelete'>Delete</button> <button onclick='addCust()' id='addCust'>Add Customer</button>");
            				btnInfo2.attr("class", "recInfo4");
            			  //APPENDING TO INVENTORY DIV	
            				buttons.append(btnInfo2);
            				
            		/////APPENDING INVENTORY INFO DIV TO CURRENT RECORD BODY DIV
            				$('#custRecInfo').append(buttons);
            				
                        })
                        .fail(function(xhr, status, errorThrown) {
                            console.log("Sorry, there was a problem!");
                        })
                        .always(function(xhr, status) {
                            console.log("The request is complete!");
                        });
                        
                    })
                });
                
                
    
    ///////////////PRINTFUNCTIONS///////////////
    
            //////// INVENTORY PRINT FUNCTION
                function userPrint(){
                    $("#userPrint").click(function(){
                       $('#userInfo').printThis({
                           importCSS: true,
                            importStyle: true,//thrown in for extra measure
                            // loadCSS: "css/layout.css",
                       });
                    })
        };
                
            //////// INVENTORY PRINT FUNCTION
                function custPrint(){
                    $("#custPrint").click(function(){
                       $('#custInfo').printThis({
                           importCSS: true,
                            importStyle: true,//thrown in for extra measure
                            // loadCSS: "css/layout.css",
                       });
                    })
                };
                
                
            
    ///////////////DOWNLOAD FUNCTIONS///////////////
    
            /////// INVENTORY DOWNLOAD PDF
                function userPDF(){
                    var userTitle= $("#userTitle").html();
                    var userName= $("#userName").html();
                    var userLName= $("#userLName").html();
                    var username= $("#username").html();
                    var clearance= $("#clearance").html();
                    
                    
                    var imgData = 'data:image/png;base64,'
                        
            
                        var doc = new jsPDF("p", "pt", "letter");
                        
                        doc.canvas.height = 200 * 11;
                        doc.canvas.width = 200 * 8.5;
                        
                         margins = {
                          top: 40,
                          bottom: 60,
                          left: 40,
                          width: 522
                        };
                    
                    /////THIS ADDS THE IMAGE
                        doc.addImage(imgData, 'JPEG', 40, 40, 90, 60)
                        
                    /////THIS IS A TITLE STYLED FOR PDF
                        doc.setFontSize(18)
                        doc.setTextColor(157, 31, 96)
                        doc.setFont('courier')
                        doc.setFontType('bold')
                        doc.text(40, 150, userTitle); //(X(SIDE TO SIDE), Y(UP DOWN), TEXT(WHAT GOES INSIDE)
            
                    /////THIS IS AN INFO PEICE STYLED FOR PDF
                        doc.setFontSize(12)
                        doc.setTextColor(0, 0, 0)
                        doc.setFont('courier')
                        doc.text(50, 180, userName);  
                        
                    /////THIS IS AN INFO PEICE STYLED FOR PDF
                        doc.setFontSize(12)
                        doc.setTextColor(0, 0, 0)
                        doc.setFont('courier')
                        doc.text(300, 180, userLName); 
                
                doc.setLineWidth(.8)
                doc.line(40, 185, 560, 185)
                        
                    /////THIS IS AN INFO PEICE STYLED FOR PDF
                        doc.setFontSize(12)
                        doc.setTextColor(0, 0, 0)
                        doc.setFont('courier')
                        doc.text(50, 205, username);  
                        
                   /////THIS IS AN INFO PEICE STYLED FOR PDF
                        doc.setFontSize(12)
                        doc.setTextColor(0, 0, 0)
                        doc.setFont('courier')
                        doc.text(300, 205, clearance);  
                        
                doc.setLineWidth(.8)
                doc.line(40, 210, 560, 210)
                        
                        // adding page
                        // doc.addPage()
                        // // adding text to the second page
                        // doc.text(20, 20, 'Do you like that?')
                        
                        doc.save("userInfo.pdf");
                        
            }
            
            /////// INVENTORY DOWNLOAD PDF
                function custPDF(){
                    
                    var custTitle= $("#custTitle").html();
                    var custName= $("#custName").html();
                    var custLoc= $("#custLoc").html();
                    var custRepName= $("#custRepName").html();
                    var custPhone= $("#custPhone").html();
                    var custEmail= $("#custEmail").html();
                    
                    console.log(custTitle);
                    console.log(custName);
                    console.log(custLoc);
                    console.log(custRepName);
                    console.log(custPhone);
                    console.log(custEmail);
                    
                    
                    var imgData = 'data:image/png;base64,'
                        
            
                        var doc = new jsPDF("p", "pt", "letter");
                        
                        doc.canvas.height = 200 * 11;
                        doc.canvas.width = 200 * 8.5;
                        
                         margins = {
                          top: 40,
                          bottom: 60,
                          left: 40,
                          width: 522
                        };
                    
                    /////THIS ADDS THE IMAGE
                        doc.addImage(imgData, 'JPEG', 40, 40, 90, 60)
                        
                    /////THIS IS A TITLE STYLED FOR PDF
                        doc.setFontSize(18)
                        doc.setTextColor(157, 31, 96)
                        doc.setFont('courier')
                        doc.setFontType('bold')
                        doc.text(40, 150, custTitle); //(X(SIDE TO SIDE), Y(UP DOWN), TEXT(WHAT GOES INSIDE)
            
                    /////THIS IS AN INFO PEICE STYLED FOR PDF
                        doc.setFontSize(12)
                        doc.setTextColor(0, 0, 0)
                        doc.setFont('courier')
                        doc.text(50, 180, custName);  
                        
                    /////THIS IS AN INFO PEICE STYLED FOR PDF
                        doc.setFontSize(12)
                        doc.setTextColor(0, 0, 0)
                        doc.setFont('courier')
                        doc.text(300, 180, custLoc); 
                
                doc.setLineWidth(.8)
                doc.line(40, 185, 560, 185)
                        
                    /////THIS IS AN INFO PEICE STYLED FOR PDF
                        doc.setFontSize(12)
                        doc.setTextColor(0, 0, 0)
                        doc.setFont('courier')
                        doc.text(50, 205, custRepName);  
                        
                  /////THIS IS AN INFO PEICE STYLED FOR PDF
                        doc.setFontSize(12)
                        doc.setTextColor(0, 0, 0)
                        doc.setFont('courier')
                        doc.text(300, 205, custPhone);  
                        
                doc.setLineWidth(.8)
                doc.line(40, 210, 560, 210)
                
                    /////THIS IS AN INFO PEICE STYLED FOR PDF
                        doc.setFontSize(12)
                        doc.setTextColor(0, 0, 0)
                        doc.setFont('courier')
                        doc.text(50, 230, custEmail);  
                        
                //   /////THIS IS AN INFO PEICE STYLED FOR PDF
                //         doc.setFontSize(12)
                //         doc.setTextColor(0, 0, 0)
                //         doc.setFont('courier')
                //         doc.text(300, 230, custName);  
                        
                doc.setLineWidth(.8)
                doc.line(40, 235, 560, 235)
                        
                        // adding page
                        // doc.addPage()
                        // // adding text to the second page
                        // doc.text(20, 20, 'Do you like that?')
                        
                        doc.save("customerInfo.pdf");
                        
                        
            }
    
    
    
    //////////////UPDATING RECORD POPUP WINDOW//////////////
    
            ////////UPDATING RECORD
                function userUpdate(){
                    $("#updateUser").attr("style","display:block;")
                    
                    //get user Id
                    var userId= $(".userId").html();
                    
                //////GET THE USER INFO TO ADD TO THE FORM B4 UPDATE
                    $.ajax({
                            type: "GET",
                            url: "getUserRec.php",
                            data: {
                                "userId":userId,
                            }
                        })
                        
                        .done(function(data) {
                            
                            var userRec = JSON.parse(data);
                            console.log(userRec);
                            console.log(userRec[0].Id);
                       
                                
                        $("#updtUserName").attr("value", userRec[0].name);
                        $("#updtUserLName").attr("value", userRec[0].lName);
                        $("#updtUsername").attr("value", userRec[0].username);
                        $("#updtUserClear").attr("value", userRec[0].clearance);
                        
                            
                        })
                        .fail(function(xhr, status, errorThrown) {
                            console.log("Sorry, there was a problem!");
                        })
                        .always(function(xhr, status) {
                            console.log("The request is complete!");
                        });
                }
                
                function updateUser(){
                    
                    var userId= $(".userId").html();
                    var name= $("#updtUserName").val();
                    var lName= $("#updtUserLName").val();
                    var userName= $("#updtUsername").val();
                    var password= $("#updtUserPass").val();
                    var clearance= $("#updtUserClear").val();
                    
                    console.log(userId);
                    console.log(name);
                    console.log(lName);
                    console.log(userName);
                    console.log(password);
                    console.log(clearance);
                    
                    ////Update the Record
                    $.ajax({
                            type: "GET",
                            url: "updateUserRec.php",
                            data: {
                                "userId":userId,
                                "name":name,
                                "lName":lName,
                                "userName":userName,
                                "password":password,
                                "clearance":clearance,
                            }
                        })
                        
                        .done(function(data) {
                            console.log("done!")
                        })
                        .fail(function(xhr, status, errorThrown) {
                            console.log("Sorry, there was a problem!");
                        })
                        .always(function(xhr, status) {
                            console.log("The request is complete!");
                        });
                    
                }
            
            ////////UPDATING RECORD
                function custUpdate(){
                    $("#updateCustomer").attr("style","display:block;")
                    
                    var custId= $(".customerId").html();
                    console.log(custId);
                    
                    $.ajax({
                            type: "GET",
                            url: "getCustRec.php",
                            data: {
                                "custId":custId
                            }
                        })
                        
                        .done(function(data) {
                            
                            var custRec = JSON.parse(data);
                            console.log(custRec);
                            console.log(custRec[0].Id);
                            
                            
                        $("#customerName").attr("value", custRec[0].name);
                        $("#custRepName").attr("value", custRec[0].repName);
                        $("#custLocation").attr("value", custRec[0].location);
                        $("#custPhone").attr("value", custRec[0].phone);
                        $("#custEmail").attr("value", custRec[0].email);
                        
                            
                        })
                        .fail(function(xhr, status, errorThrown) {
                            console.log("Sorry, there was a problem!");
                        })
                        .always(function(xhr, status) {
                            console.log("The request is complete!");
                        });
                    
                }
                
                function updateCustomer(){
                    
                    var custId= $(".customerId").html();
                    var custName= $("#customerName").val();
                    var custRepName= $("#customerRepName").val();
                    var custLocation= $("#customerLocation").val();
                    var custPhone= $("#customerPhone").val();
                    var custEmail= $("#customerEmail").val();
                    
                    console.log(custId);
                    console.log(custName);
                    console.log(custRepName);
                    console.log(custLocation);
                    console.log(custPhone);
                    console.log(custEmail);
                    
                    ////Update the Record
                    $.ajax({
                            type: "GET",
                            url: "updateCustRec.php",
                            data: {
                                "custId":custId,
                                "custName":custName,
                                "custRepName":custRepName,
                                "custLocation":custLocation,
                                "custPhone":custPhone,
                                "custEmail":custEmail,
                            }
                        })
                        
                        .done(function(data) {
                            console.log("done!")
                        })
                        .fail(function(xhr, status, errorThrown) {
                            console.log("Sorry, there was a problem!");
                        })
                        .always(function(xhr, status) {
                            console.log("The request is complete!");
                        });
                    
                }
                
                
                
    //////////////DELETING AND ADDING RECORDS//////////////
            
            /////////DELETING INVENTORY RECORD - FUNCTION
                    function userDelete(){
                        
            			var invId=$('#invId').html();
            			
            			var response=confirm ("Are You Sure You Want To Delete "+ $('#invComod').html());
            			
                        if (response == true) {
                            
                            $.ajax({
                                
                                type: "GET",
                                url: "getInventoryRecord.php",
                                data: {
                                    "lotId":invId,
                                }
                            })
                            .done(function(data) {
                                
                                var inventoryRec = JSON.parse(data);
                                console.log(inventoryRec);
                                console.log(inventoryRec[0].Id);
                                
                                var invId=inventoryRec[0].Id;
                                var invCommodity=inventoryRec[0].commodity_Id;
                                var invAmount=inventoryRec[0].amount;
                                var invOwner=inventoryRec[0].custName;
                                var date= moment().format('YYYY-MM-DD');
                                var time= moment().format('hh:mm:ss');
                            
                                $.ajax({
                                    type: "GET",
                                    url: "deleteInvRec.php",
                                    data: {
                                        "invId":invId,
                                        "invCommodity":invCommodity,
                                        "invAmount":invAmount,
                                        "invOwner":invOwner,
                                        "date":date,
                                        "time":time,
                                    }
                                    }) 
                                    .done(function(data) {
                                        console.log("done!");
                                    })
                                    .fail(function(xhr, status, errorThrown) {
                                        console.log("Sorry, there was a problem!");
                                    })
                                    .always(function(xhr, status) {
                                        console.log("The request is complete!");
                                     });
                                
                                })
                                .fail(function(xhr, status, errorThrown) {
                                    console.log("Sorry, there was a problem!");
                                })
                                .always(function(xhr, status) {
                                    console.log("The request is complete!");
                                });
                                
                                    
                            } else {
                                console.log("no");
                            }
                        }
                        
            /////////DELETING INVENTORY RECORD - FUNCTION
                    function custDelete(){
                        
            			var invId=$('#invId').html();
            			
            			var response=confirm ("Are You Sure You Want To Delete "+ $('#invComod').html());
            			
                        if (response == true) {
                            
                            $.ajax({
                                
                                type: "GET",
                                url: "getInventoryRecord.php",
                                data: {
                                    "lotId":invId,
                                }
                            })
                            .done(function(data) {
                                
                                var inventoryRec = JSON.parse(data);
                                console.log(inventoryRec);
                                console.log(inventoryRec[0].Id);
                                
                                var invId=inventoryRec[0].Id;
                                var invCommodity=inventoryRec[0].commodity_Id;
                                var invAmount=inventoryRec[0].amount;
                                var invOwner=inventoryRec[0].custName;
                                var date= moment().format('YYYY-MM-DD');
                                var time= moment().format('hh:mm:ss');
                            
                                $.ajax({
                                    type: "GET",
                                    url: "deleteInvRec.php",
                                    data: {
                                        "invId":invId,
                                        "invCommodity":invCommodity,
                                        "invAmount":invAmount,
                                        "invOwner":invOwner,
                                        "date":date,
                                        "time":time,
                                    }
                                    }) 
                                    .done(function(data) {
                                        console.log("done!");
                                    })
                                    .fail(function(xhr, status, errorThrown) {
                                        console.log("Sorry, there was a problem!");
                                    })
                                    .always(function(xhr, status) {
                                        console.log("The request is complete!");
                                     });
                                
                                })
                                .fail(function(xhr, status, errorThrown) {
                                    console.log("Sorry, there was a problem!");
                                })
                                .always(function(xhr, status) {
                                    console.log("The request is complete!");
                                });
                                
                                    
                            } else {
                                console.log("no");
                            }
                        }
    
    //////////////UPDATING RECORD POPUP WINDOW//////////////
    
            ////////ADDING USER RECORD
                function addUser(){
                    $("#addUser").attr("style","display:block;")
                    
                }
                
                function insertNewUser(){
                    
                    var name= $("#updtUserName2").val();
                    var lName= $("#updtUserLName2").val();
                    var userName= $("#updtUsername2").val();
                    var password= $("#updtUserPass2").val();
                    var clearance= $("#updtUserClear2").val();
                    
                    console.log(name);
                    console.log(lName);
                    console.log(userName);
                    console.log(password);
                    console.log(clearance);
                    
                    ////Update the Record
                    $.ajax({
                            type: "GET",
                            url: "addUser.php",
                            data: {
                                "name":name,
                                "lName":lName,
                                "userName":userName,
                                "password":password,
                                "clearance":clearance,
                            }
                        })
                        
                        .done(function(data) {
                            console.log("done!")
                        })
                        .fail(function(xhr, status, errorThrown) {
                            console.log("Sorry, there was a problem!");
                        })
                        .always(function(xhr, status) {
                            console.log("The request is complete!");
                        });
                    
                }
            
            ////////ADDING CUSTOMER RECORD
                function addCust(){
                    $("#addCust").attr("style","display:block;")
                    
                }
                
                function insertNewCust(){
                    
                    var custName= $("#custName2").val();
                    var custRepName= $("#custRepName2").val();
                    var custLocation= $("#custLocation2").val();
                    var custPhone= $("#custPhone2").val();
                    var custEmail= $("#custEmail2").val();
                    
                    console.log(custName);
                    console.log(custRepName);
                    console.log(custLocation);
                    console.log(custPhone);
                    console.log(custEmail);
                    
                    ////Update the Record
                    $.ajax({
                            type: "GET",
                            url: "addCust.php",
                            data: {
                                "custName":custName,
                                "custRepName":custRepName,
                                "custLocation":custLocation,
                                "custPhone":custPhone,
                                "custEmail":custEmail,
                            }
                        })
                        
                        .done(function(data) {
                            console.log("done!")
                        })
                        .fail(function(xhr, status, errorThrown) {
                            console.log("Sorry, there was a problem!");
                        })
                        .always(function(xhr, status) {
                            console.log("The request is complete!");
                        });
                    
                }
                        
            
    </script>
</html>